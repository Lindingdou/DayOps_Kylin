...
**文件（6）**：新建✅(完整文档重置) 打开✅(.pmx) 保存✅(.pmx) 另存为✅(.pmx/.dxf/.dwg) 导出✅(场景实体级 DXF，保留图层) 导入✅(DXF/DWG/OFF + 点CSV/TXT；见格式矩阵) | 选项❌
...
**修改（9）**：复制✅ 移动✅ 旋转✅ 删除✅ 分解✅ 偏移✅(线/圆/矩/多段线/圆弧) 打断✅(线/多段线/圆弧) 夹点✅ 修剪✅ 延伸✅(边界任意实体，目标仍限直线) | (目标为多段线/圆弧的修剪待做)
**图层（5）**：新建图层✅ 全开✅ 冻结✅ 锁定✅(作用当前层，渲染+拾取遵守) | 图层特性管理器🟡(循环切当前，无完整特性表/颜色线型面板)
**视图（5）**：2D✅ 3D✅ | 缩放🟡(我做的是实体缩放，视图组此按钮语义应为视图缩放，**待核**) · 清空视图🟡 | 渲染配置❌
**特性/选择（4）**：全部选择✅ · 最后✅ · 上次✅ · 窗口框选✅(窗口/交叉) · 夹点✅ | 快速选择❌
...
**下一步（按可做性排序，续 loop）**：圆 TTR + 圆下拉 UI → 绘制图层完整面板 → 目标为多段线/圆弧的修剪 → 3D 框选 → 文字/渲染配置(需 Skia/内核，记录)。
