...
- [ ] 图层细化：绘制层的**完整面板**（列 `_layers` + 每层显隐/冻结/锁定/改色，与导入层面板合并；当前作用于"当前层"，靠图层特性管理器循环切换）
...
**文件（6）**：新建✅(完整文档重置) 打开✅(.pmx) 保存✅(.pmx) | 另存为🟡(现=导出DXF，缺 .pmx 另存为) · 导入🟡(DXF/DWG/OFF + 点CSV/TXT；见下方格式矩阵) · 选项❌

**📂 文件格式矩阵**（原版导入清单见 Explore 调查；打开=.pmx/.pmb，导入=交换格式）：
| 格式 | 状态 | 说明 |
|---|---|---|
| `.dxf` | ✅ | ACadSharp；Line/Poly/Circle/Arc/Point/Ellipse/Insert/Spline（缺文字/填充/标注） |
| `.dwg` | ✅ | ACadSharp DwgReader；与 DXF 同管线（本平台读写已验证） |
...
**绘制（9）**：点✅ 多段线✅ 直线✅ 矩形✅ 正多边形✅ 滑动多段线✅ | 圆🟡(圆心-半径✅ + 2P✅ + 3P✅，缺 TTR + UI下拉；命令 `C2P`/`C3P`) · 圆弧🟡(仅三点) | **文字❌**(需 Skia)
**修改（9）**：复制✅ 移动✅ 旋转✅ 删除✅ 分解✅ | 延伸🟡(仅线) · 修剪🟡(仅线) · 偏移🟡(仅线/圆/矩) · 打断🟡(仅线)
**图层（5）**：新建图层✅ 全开✅ 冻结✅ 锁定✅(作用当前层，渲染+拾取遵守) | 图层特性管理器🟡(循环切当前，无完整特性表/颜色线型面板)
**视图（5）**：2D✅ 3D✅ | 缩放🟡(我做的是实体缩放，视图组此按钮语义应为视图缩放，**待核**) · 清空视图🟡 | 渲染配置❌
**特性/选择（4）**：全部选择✅ · 最后✅ · 上次✅ | 快速选择❌
**帮助（2）**：帮助文档❌ · 注册❌
...
3. **导入与绘制两套、部分相通**：CAD(DXF/DWG/OFF) 导入几何仍是显示态、不可编辑；但**点数据导入已进可编辑场景**。CAD 导入→可编辑实体仍待做（需实体级往返，或内核 AcDb 对接）。
4. **夹点编辑（grip）没做**：选中后无夹点拖拽。
...
**下一步（按可做性排序，续 loop）**：夹点编辑 → CAD 导入几何转可编辑实体 → 多段线/圆弧的偏移·打断 → 圆 TTR + 圆下拉 UI → 绘制图层完整面板 → .pmx 另存为 → 文字/渲染配置(需 Skia/内核，记录)。
