...
**绘制（9）**：点✅ 多段线✅ 直线✅ 矩形✅ 正多边形✅ 滑动多段线✅ 圆✅(圆心半径/2P/3P/TTR) 圆弧✅(三点/SCE/CSE，下拉可达；SER 等待做) | **文字❌**(需 Skia)
...
**下一步（按可做性排序，续 loop）**：目标为多段线/圆弧的修剪 → 3D 框选 → 图层 ByLayer 色模型+改色 → 圆弧 SER/圆 圆-圆相切 → 文字/渲染配置(需 Skia/内核，记录)。
