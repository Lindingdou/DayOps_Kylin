...
- [记录] **粗糙度/曲率分析(点云 §二)**：`SurfaceAttribResult` 表明二者为内核原生(PMSA 二进制, 逐顶点属性写 mesh uv 通道), **精确逐顶点公式不在托管源**(粗糙度=邻域平面拟合残差? 曲率=角亏 Gaussian? 法向变分 mean? 内核未暴露)——忠实移植无据, 记录待上机。坡度/坡向/高程着色/两期算量已由 `TerrainAnalysis` 覆盖。
