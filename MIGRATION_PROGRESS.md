...
- [x] **内部一致性修复：CommandCatalog 死项**(`本次`)：另开**内部一致性**角度(脚本核 207 目录项在处理器全文出现次数)，抓到 3 个**声明但不解析**的死项——目录写裸名但处理器用长名：`坡度`(处理器仅 `坡度着色`)、`坡向`(仅 `坡向着色`)、`资源量`(仅 `资源量估算/报告`)。点自动补全/目录这些裸项**静默无反应**(真 bug)。修：各加为既有处理器别名(坡度/坡向→对应着色，资源量→资源量估算 ResourceReport)。脚本复验 **207/207 全解析**。→ 命令目录与分派**完全自洽**，无悬空项。
