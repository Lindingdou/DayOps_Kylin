...
**绘制（9）**：点✅ 多段线✅ 直线✅ 矩形✅ 正多边形✅ 滑动多段线✅ | 圆🟡(仅圆心-半径，缺 2P/3P/TTR) · 圆弧🟡(仅三点) | **文字❌**(需 Skia)
**修改（9）**：复制✅ 移动✅ 旋转✅ 删除✅ 分解✅ | 延伸🟡(仅线) · 修剪🟡(仅线) · 偏移🟡(仅线/圆/矩) | **打断❌**
...
2. **精确键盘输入全缺**：输坐标(10,20)/距离/角度——CAD 精绘核心，完全没做。
3. **导入与绘制两套、互不相通**：导入的 DXF 几何不是可编辑场景实体（不能选中/编辑/删除/捕捉端点之外的操作）。
4. **夹点编辑（grip）没做**：选中后无夹点拖拽。
5. **命令行只识别固定词**：无参数解析、无历史、无 Enter 重复上次命令。

**已完成（续 loop）**：全部选择/最后/上次 ✅ · 分解 ✅ · 正多边形 ✅ · 滑动多段线 ✅ · 橡皮筋预览(全绘制工具) ✅

**下一步（按可做性排序，续 loop）**：精确坐标输入(中，改交互) → 打断(BREAK) → 圆 2P/3P/TTR → 图层特性(冻结/锁定/全开) → 文字/渲染配置(需 Skia/内核，记录)。
