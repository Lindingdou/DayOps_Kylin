...
- **★数据导入脉络完成(9 类)**：设备域 5(DataImportCenter: 生产记录/产能/故障/KPI/台账) + 地质 3(煤质化验/观测点/见煤成果) + 计划 1(月度计划)，均 CSV 基础、可验证(导入后查库行)、参数化防注入、FK 约束正确生效。**覆盖全部可分析数据**：地质钻孔(煤质/观测点/见煤成果)配对 见煤统计/层位展点/可采区域/煤质分析；设备域配对 台账/产能/KPI/故障统计;计划配对 达成度。余表(边坡/路况——纯展示数据)可循框架平凡扩展但价值低。**数据导入通用框架(ParseCsvRows + ImportCsvToDbAsync + 9 ImportXxx)完备**。674 测试。
