...
**绘制（9）**：点✅ 多段线✅ 直线✅ 矩形✅ 正多边形✅ 滑动多段线✅ 圆✅(圆心半径/2P/3P/TTR，下拉可达；TTR 限线-线，圆-圆/线-圆待做) | 圆弧🟡(仅三点) | **文字❌**(需 Skia)
**修改（9）**：复制✅ 移动✅ 旋转✅ 删除✅ 分解✅ 偏移✅(线/圆/矩/多段线/圆弧) 打断✅(线/多段线/圆弧) 夹点✅ 修剪✅ 延伸✅(边界任意实体，目标仍限直线) | (目标为多段线/圆弧的修剪待做)
**图层（5）**：新建图层✅ 全开✅ 冻结✅ 锁定✅ 完整面板✅(每层 显隐/冻结/锁定/设当前/色块, 渲染+拾取遵守) | 图层改色🟡(需 ByLayer 色模型) · 线型❌
...
**下一步（按可做性排序，续 loop）**：圆弧多画法(起点/圆心/端点等) → 目标为多段线/圆弧的修剪 → 3D 框选 → 图层 ByLayer 色模型+改色 → 文字/渲染配置(需 Skia/内核，记录)。
