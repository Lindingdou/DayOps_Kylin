...
**已完成（续 loop）**：全部选择/最后/上次 ✅ · 分解 ✅ · 正多边形 ✅ · 滑动多段线 ✅ · 橡皮筋预览(全绘制工具) ✅ · 精确坐标输入(绝对/相对/极) ✅ · 打断(直线) ✅

**下一步（按可做性排序，续 loop）**：圆 2P/3P/TTR → 图层特性(冻结/锁定/全开) → 夹点编辑 → 多段线/圆弧的偏移·打断 → 文字/渲染配置(需 Skia/内核，记录)。
