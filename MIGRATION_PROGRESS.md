...
**修改（9）**：复制✅ 移动✅ 旋转✅ 删除✅ 分解✅ 偏移✅(线/圆/矩/多段线/圆弧) 打断✅(线/多段线/圆弧) 夹点✅ | 延伸🟡(仅线) · 修剪🟡(仅线)
...
**下一步（按可做性排序，续 loop）**：修剪/延伸支持多段线/圆弧 → 圆 TTR + 圆下拉 UI → 绘制图层完整面板 → .pmx 另存为 → 3D 框选 → 文字/渲染配置(需 Skia/内核，记录)。
