...
- [第二轮普查·记录] **2D 场景 Z 阻塞项(线/点编辑)**：统一线高程(POLYUNIFYZ)/修改高程点(POINTSETZ)/赋节点高程(AssignZByFormula z=aX+bY+c)/点落到面(POINTPROJECT)/线落到面(POLYPROJECT)——原为 `IPolylineOpsCapability`/`IEntityCapability` 逐点 Z 操作, 但 **Kylin 场景为 2D(折线=(x,y)、点=X,Y 无逐点 Z)**, 无 Z 可改/可投影, 记录待 3D 场景。(点/线落面的网格 Z 重心采样器已在 `MineableAreaIdentifier.SampleMeshZ` 具备, 缺的是场景侧逐点 Z 载体。)竖曲线平滑 `ProfileSmoother.VerticalCurves`(83 行纯核, (s,z) 剖面抛物线竖曲线) 弱映射(非 227 项独立按钮, 属斜坡道纵断面子算法), 记录备选。
- [x·部分] **演化对比 误接修复(运输 §六)**：已从点云 C2C 别名移除 `演化对比`(commit `8894be6`)——止损其错误执行点云比对。**真实现待专项移植**：`RoadLib.Evolution.RoadEvolutionAnalyzer`(323 行) 依赖深链 `GeometryIndex`(106)→`SegmentGrid`(86) + `PolylineMetrics`(Resample/SampleTangent/SubPolyline/Length2D) + `RoadEvolutionResult`(162: RouteEvolution/Ledger/RoadEvolutionClass/EvolutionSide) + `RoadEvolutionOptions` + RoadGraph/RoadEdge + Point3d，共 **6–7 文件 ~800 行**, RE1-8 逐段游程匹配规则深度交织。**裁定暂缓**(忠实优先于凑数)：超单 tick 责任范围、跨文件转写失真风险高；误接 bug 已修止损, 全链记录为**唯一剩余大型待办**(需两期中线 CSV 输入设计 + 全链忠实移植 + 构造两期已知关系分类测试)。
