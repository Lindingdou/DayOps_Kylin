...
**绘制（9）**：点✅ 多段线✅ 直线✅ 矩形✅ | 圆🟡(仅圆心-半径，缺 2P/3P/TTR) · 圆弧🟡(仅三点) | 滑动多段线❌ · **正多边形❌** · **文字❌**(需 Skia)
...
**下一步（按可做性排序，续 loop）**：全部选择/最后/上次(易) → 正多边形/分解(中) → 精确坐标输入(中，改交互) → 打断 → 圆 2P/3P → 图层特性(冻结/锁定/全开) → 文字/渲染配置(需 Skia/内核，记录)。
