...
- [x] **补齐全 6 煤质分析结果导出**(`8fb8613`)：续补 品位-储量曲线/分标高/洗选/用途 的 ToCsv + 导出命令，加通用 `SaveCsvAsync` 辅助。至此 CoalQualityAnalytics **全 6 分析结果均可导出 CSV**(符合性/离群/品位-储量/分标高/洗选/用途)。+1 单测(4 序列化)。→ 煤质分析(最富——含逐段/逐层明细)的结果导出能力**已完备**；fleet 优化/forecast 路径等摘要类结果其状态栏文本已足，导出模式可循 `SaveCsvAsync`+`ToCsv` 平凡扩展。665 测试。
