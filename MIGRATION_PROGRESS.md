...
- [记录·待修] **演化对比 误接 bug(运输 §六)**：`演化对比` 现被错接到 `CloudCompareAsync`(点云 C2C, 行 657), 而 §六 演化对比 应比对两期路网中心线。真实现 `RoadLib.Evolution.RoadEvolutionAnalyzer`(323 行, 纯托管但依赖 RoadGraph/GeometryIndex 类型)。下一 tick 修正接线并移植。
