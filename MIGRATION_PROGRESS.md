...
**文件（6）**：新建✅ 打开✅ 保存✅ | 另存为🟡(现=导出DXF，缺 .pmx 另存为) · 导入🟡(仅 Line/Poly/Circle/Arc/Point/Ellipse/Insert/Spline；缺**文字/填充/标注/DWG**；导入几何**不可编辑**) · 选项❌
...
**图层（5）**：新建图层✅ | 图层特性管理器🟡(仅循环切当前，无特性表/颜色线型冻结锁定) | 全开❌ · 冻结❌ · 锁定❌
...
3. **导入与绘制两套、互不相通**：导入的 DXF 几何不是可编辑场景实体（不能选中/编辑/删除/捕捉端点之外的操作）。
...
**下一步（按可做性排序，续 loop）**：夹点编辑 → 多段线/圆弧的偏移·打断 → 圆 TTR + 圆下拉 UI → 绘制图层面板(列 _layers + 每层开关/冻结/锁定，与导入层面板合并) → 文字/渲染配置(需 Skia/内核，记录)。
