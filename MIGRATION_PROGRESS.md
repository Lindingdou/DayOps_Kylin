...
**文件（6）**：新建✅(完整重置) 打开✅(.pmx,记路径) 保存✅(回写当前文件,首次弹框) 另存为✅(.pmx/.dxf/.dwg) 导出✅(场景实体级,保留图层) 导入✅(DXF/DWG/OFF+点CSV/TXT) | 选项❌(设置对话框待做)
...
**下一步（仅对齐原程序的增量增强，不新增原版没有的命令）**：选项设置对话框(网格/捕捉容差等) → 圆弧目标修剪 → 文字/渲染配置(需 Skia/内核，记录)。
