...
public class SceneIOTests
{
    ...
    {
        var s = new Scene();
        s.Add(new LineEntity { X0 = 0, Y0 = 0, X1 = 10, Y1 = 0, Dash = DashPattern.ByName("虚线"), LineWeight = 25, Transparency = 40, Visible = false });
        ...
        Assert.Equal(40, line.Transparency);            // 透明度持久化
    ...
    public void Old_pmx_without_style_fields_loads_with_defaults()
    {
        ...
        var line = Assert.IsType<LineEntity>(s2.Entities[0]);
        Assert.Null(line.Dash); Assert.Equal(-1, line.LineWeight); Assert.Equal(-1, line.Transparency); Assert.True(line.Visible);
    }
