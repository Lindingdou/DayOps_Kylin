...
/// 各导入自有测用手写内存 schema, 测不到迁移一致性; 此测覆盖 9 自足导入 + blast/equipment(GeoQueryAggregationTests 守卫)
/// = 13 中 11。余 CoalSamples/SeamResults 需 borehole 父行(hole_id→borehole_id NOT NULL 查找), 其列-schema 由逻辑测+生产覆盖。
/// </summary>
