-- 由 build/sqlite2pg.py 从 V035_sink_profile_face_routing.sql 自动生成, 请勿手改。
...
CREATE TABLE IF NOT EXISTS sink_stocktake (
    ...
    after_filled_m3  DOUBLE PRECISION   NOT NULL DEFAULT 0,           -- 盘点后已填(占容方 m³)
    delta_m3        DOUBLE PRECISION    NOT NULL DEFAULT 0,           -- 差额 = after - before(正=补记,负=冲回)
...
CREATE TABLE IF NOT EXISTS working_face_routing (
    ...
    bench_elevation_m       DOUBLE PRECISION    NOT NULL DEFAULT 0,   -- 台阶标高 m(不是台阶高度!)
