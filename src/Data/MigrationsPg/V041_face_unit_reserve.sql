-- 由 build/sqlite2pg.py 从 V041_face_unit_reserve.sql 自动生成, 请勿手改。
...
ALTER TABLE working_face_routing ADD COLUMN unit_id TEXT NOT NULL DEFAULT '';  -- 采掘单元号(= 采矿模型 UnitId);空=未绑
ALTER TABLE working_face_routing ADD COLUMN available_reserve_m3 DOUBLE PRECISION NOT NULL DEFAULT 0;   -- 备采储量(m³ 原位实方);0=未录,非"采空"
ALTER TABLE working_face_routing ADD COLUMN advance_azimuth_deg DOUBLE PRECISION;                      -- 推进方位(°,正北起顺时针);NULL=未录
ALTER TABLE working_face_routing ADD COLUMN mining_width_m DOUBLE PRECISION;                      -- 本面当前推进宽(m);NULL=未录
