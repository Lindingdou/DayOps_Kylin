-- 由 build/sqlite2dm.py 从 V036_sink_xyz_face_quality_trucks.sql 自动生成, 请勿手改。
...
ALTER TABLE sink_profile ADD COLUMN x DOUBLE NOT NULL DEFAULT 0;   -- 去向代表点 X(m);x、y 均为 0 = 未录坐标
ALTER TABLE sink_profile ADD COLUMN y DOUBLE NOT NULL DEFAULT 0;   -- 去向代表点 Y(m)
ALTER TABLE sink_profile ADD COLUMN z DOUBLE NOT NULL DEFAULT 0;   -- 去向代表点 Z 标高(m);Z=0 是合法标高,不参与"有没有坐标"的判定

-- ── ② 作业面煤质目标(配煤约束输入;NULL = 该面无煤质目标,即纯量矿)──────────────
ALTER TABLE working_face_routing ADD COLUMN quality_ash_pct DOUBLE;   -- 灰分目标 %(NULL=无目标)
ALTER TABLE working_face_routing ADD COLUMN quality_cv_mjkg DOUBLE;   -- 热值目标 MJ/kg(NULL=无目标)
ALTER TABLE working_face_routing ADD COLUMN quality_sulfur_pct DOUBLE;   -- 硫分目标 %(NULL=无目标)
ALTER TABLE working_face_routing ADD COLUMN quality_moisture_pct DOUBLE;   -- 水分目标 %(NULL=无目标)

-- ── ③ 作业面实配车号(现场实际配的车;与建议车数之差 = 运力不足的判据)────────────
ALTER TABLE working_face_routing ADD COLUMN assigned_trucks VARCHAR(255) NOT NULL DEFAULT '';  -- 车号逗号分隔,如 'T-01,T-02';空=未配车
