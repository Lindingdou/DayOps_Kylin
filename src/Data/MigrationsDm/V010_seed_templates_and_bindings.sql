-- 由 build/sqlite2dm.py 从 V010_seed_templates_and_bindings.sql 自动生成, 请勿手改。
...
-- ─── 3 套现行模板 ───────────────────────────────────────────────────────────
REPLACE INTO process_template
    (template_id, code, name, description, applicable_material, applicable_hardness, version, is_current, "status") VALUES
    ...
    (3, 'coal_soft_v1.0',   '煤层标准 v1.0',    '纯煤层直采,无爆破',                  'coal', 'soft',   'v1.0', 1, 'active');

-- ─── 硬岩区模板的参数推荐值 ────────────────────────────────────────────────
-- 钻孔环节(101)
REPLACE INTO template_param_value (template_id, param_id, recommended_value, min_value, max_value, notes) VALUES
    (1, 1001, 250, 220, 280, '硬岩取大孔径'),
    (1, 1002, 15,  13,  17,  '台阶 15m + 超钻 1.5m'),
    (1, 1003, 7,   6,   8,   '匹配孔径与单耗'),
    (1, 1004, 6,   5,   7,   '梅花布孔'),
    (1, 1005, 1.5, 1,   2,   '抗反弹超钻'),
    (1, 1006, 0,   NULL, NULL, '垂直钻孔');

-- 装药 102
REPLACE INTO template_param_value (template_id, param_id, recommended_value, min_value, max_value, notes) VALUES
    (1, 1101, 8.5, 7, 10, '硬岩高密度装药'),
    (1, 1102, 4,   3, 5,  '足够堵塞抑制飞石');
...
REPLACE INTO template_param_value (template_id, param_id, recommended_value, min_value, max_value, notes) VALUES
    ...
    (1, 2005, 50, 40, 60, '采宽兼顾效率与安全');

-- 剥离 202
REPLACE INTO template_param_value (template_id, param_id, recommended_value, min_value, max_value, notes) VALUES
    (1, 2101, 45, 30, 60, '硬岩月推进度');

-- 主道运输 302
REPLACE INTO template_param_value (template_id, param_id, recommended_value, min_value, max_value, notes) VALUES
    (1, 3001, 4,  2, 6,  '安太堡矿平均运距'),
    (1, 3002, 8,  6, 10, '主道最大坡度'),
    (1, 3003, 36, 30,40, '930E 车宽 + 2 倍裕度');
...
REPLACE INTO phase_location_binding
    (phase_id, location_code, bound_template_id, is_active, started_at) VALUES
    -- 1195 平盘
