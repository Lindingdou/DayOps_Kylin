-- 由 build/sqlite2dm.py 从 V045_road_centerline_set.sql 自动生成, 请勿手改。
...
CREATE TABLE road_centerline_set (
    id           INT IDENTITY(1,1) PRIMARY KEY,
    name         VARCHAR(255)    NOT NULL DEFAULT '',       -- 存档名(可含时期,如"2026-06 提取+人工整理")
    captured_at  VARCHAR(255)    NOT NULL DEFAULT '',       -- 存档时刻(app 写,"什么时候的中线")
    "source"       VARCHAR(255)    NOT NULL DEFAULT '',       -- 来路标注:提取/手动/管理整理…(纯留痕,不参与逻辑)
    geometry_b64 VARCHAR(2000)    NOT NULL DEFAULT '',       -- GZip 二进制 base64(RCL1),opaque
    line_count   INT NOT NULL DEFAULT 0,
    vertex_count INT NOT NULL DEFAULT 0,
    length_km    DOUBLE    NOT NULL DEFAULT 0,        -- 三维总长 km(冗余,列表显示用)
