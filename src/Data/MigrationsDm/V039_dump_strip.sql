-- 由 build/sqlite2dm.py 从 V039_dump_strip.sql 自动生成, 请勿手改。
...
CREATE TABLE dump_strip (
    id                  INT IDENTITY(1,1) PRIMARY KEY,

    region_id           INT NOT NULL,           -- → mineable_region.id(排土场)
    region_name         VARCHAR(255)    NOT NULL DEFAULT '',-- 冗余名字:清单直接可读,不必每次连表
    category            VARCHAR(255)    NOT NULL DEFAULT 'external_dump',  -- external_dump / internal_dump

    code                VARCHAR(255)    NOT NULL,           -- 位置编号 '外排1-L3-P02-S05'
    level_index         INT NOT NULL,           -- 台阶级序(1 = 最上一级)
    panel_index         INT NOT NULL,           -- 沿走向第几幅(1 起)
    panel_count         INT NOT NULL DEFAULT 1, -- 本级共几幅
    step_index          INT NOT NULL,           -- 沿推进方向第几带(1 = 当前排土线那一带)

    crest_z             DOUBLE    NOT NULL,           -- 坡顶标高(m)
    toe_z               DOUBLE    NOT NULL,           -- 坡底标高(m)
    bench_height_m      DOUBLE    NOT NULL,           -- 台阶高 = crest_z − toe_z

    strike_len_m        DOUBLE    NOT NULL,           -- 走向长(m,水平)
    strip_width_m       DOUBLE    NOT NULL,           -- 排土条带宽度 W(m,推进方向)
    capacity_m3         DOUBLE    NOT NULL,           -- 库容(m³,占容方)= 走向长 × W × 台阶高

    centroid_x          DOUBLE    NOT NULL,           -- 质心(世界坐标)—— 寻径算运距吃它
    centroid_y          DOUBLE    NOT NULL,
    centroid_z          DOUBLE    NOT NULL,

    -- 前脸轨(扁平 xyz JSON):重建体/重画不必再跑一遍切分。存轨不存体 ——
    -- 体在图上(entity_handle),库里存能复现体的最小输入。
    crest_json          VARCHAR(2000)    NOT NULL DEFAULT '[]',
    toe_json            VARCHAR(2000)    NOT NULL DEFAULT '[]',

    entity_handle       INT NOT NULL DEFAULT 0, -- 图上壳子体的 handle(0 = 还没建体)

    design_version      VARCHAR(255),                       -- 多方案比选:同一排土场并存几种切法
    notes               VARCHAR(2000),
...
CREATE UNIQUE INDEX idx_dump_strip_key
    ON dump_strip(region_id, NVL(design_version, ''), level_index, panel_index, step_index);
