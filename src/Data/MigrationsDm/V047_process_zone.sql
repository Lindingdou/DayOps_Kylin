-- 由 build/sqlite2dm.py 从 V047_process_zone.sql 自动生成, 请勿手改。
...
CREATE TABLE process_zone (
    id                 INT IDENTITY(1,1) PRIMARY KEY,
    period             VARCHAR(255) NOT NULL,                  -- 期次 yyyy-MM(工序区是按期的)
    process            VARCHAR(255) NOT NULL,                  -- 工序码:drill/blast_guard/load/dump_tip/dump_doze
    name               VARCHAR(255) NOT NULL,                  -- 区域名(作业面名/炮次号;分块带 -2 -3 后缀)
    group_key          VARCHAR(255) NOT NULL DEFAULT '',       -- 分组键(作业面名;警戒区按炮次)
    piece_index        INT NOT NULL DEFAULT 1,     -- 本组第几块(不相连时分块出,Z6)
    piece_count        INT NOT NULL DEFAULT 1,

    points_json        VARCHAR(2000) NOT NULL,                  -- 扁平 [x0,y0,z0,x1,y1,z1,...]，与 mineable_region 同口径
    z_source           VARCHAR(255) NOT NULL,                  -- 高程出处(空 = 不许入库)
    cell_m             DOUBLE NOT NULL,                  -- 栅格格距 m = 轮廓精度
    area_m2            DOUBLE NOT NULL DEFAULT 0,        -- 栅格面积(不是描边环面积)
    ring_area_m2       DOUBLE NOT NULL DEFAULT 0,        -- 描边环面积(与上面差得多 = 描边不可信)
    clipped_at_border  INT NOT NULL DEFAULT 0,     -- 1 = 膨胀顶到格网边界，本区被截断

    unit_ids           VARCHAR(255) NOT NULL DEFAULT '',       -- 覆盖的采掘单元号，顿号分隔
    unit_count         INT NOT NULL DEFAULT 0,
    volume_m3          DOUBLE,                           -- 本区的量(NULL=不适用,如警戒区)
    volume_basis       VARCHAR(255) NOT NULL DEFAULT '',       -- 量口径:原位实方/控制方量/排弃占容(三者绝不许并成一列)
    equip_role         VARCHAR(255) NOT NULL DEFAULT '',       -- 该区的设备角色:钻机/电铲/卡车/推土机

    lead_days          DOUBLE,                           -- 相对采装的超前(+)/滞后(-)工日;NULL=不适用
    guard_radius_m     DOUBLE,                           -- 警戒半径 m(仅 blast_guard)
    band_width_m       DOUBLE,                           -- 带宽 m(仅 dump_tip/dump_doze)

    active             INT NOT NULL DEFAULT 1,     -- 本期算不算数(与 visible 是两件事)
    visible            INT NOT NULL DEFAULT 1,     -- 画不画它
    note               VARCHAR(2000) NOT NULL DEFAULT '',       -- 记账:期次+分组+单元清单+格距+Z出处
