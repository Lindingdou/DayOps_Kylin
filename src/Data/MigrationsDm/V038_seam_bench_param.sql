-- 由 build/sqlite2dm.py 从 V038_seam_bench_param.sql 自动生成, 请勿手改。
...
CREATE TABLE seam_bench_param (
    id                      INT IDENTITY(1,1) PRIMARY KEY,
    seam_code               VARCHAR(255)    NOT NULL,               -- → coal_seam_def.code (4 / 9 / 11 ...)

    -- 覆盖 parameter_definition 的煤台阶三项(NULL = 不覆盖,回落全矿默认)
    bench_height_m          DOUBLE,                           -- 覆盖 coal_bench_height (2007)
    bench_slope_angle_deg   DOUBLE,                           -- 覆盖 coal_bench_slope_angle (2008)
    berm_width_m            DOUBLE,                           -- 覆盖 coal_platform_width (2009)

    -- 煤的采矿模型专有(parameter_definition 里没有对应项)
    strip_width_m           DOUBLE,                           -- 采掘带宽度 W(往高墙里推进的水平深度)
    layering_mode           VARCHAR(255)    NOT NULL DEFAULT 'inclined',  -- 'inclined' 倾斜分层 / 'horizontal' 顶底水平
    min_mineable_thick_m    DOUBLE,                           -- 最小可采厚:薄于此不出煤体
    datum                   VARCHAR(255)    NOT NULL DEFAULT 'floor',     -- 'floor' 从底板起算 / 'roof' 从顶板起算

    design_version          VARCHAR(255),
    notes                   VARCHAR(2000),
...
CREATE UNIQUE INDEX idx_seam_bench_param_key
    ON seam_bench_param(seam_code, NVL(design_version, ''));
...
INSERT IGNORE INTO seam_bench_param
