-- 由 build/sqlite2dm.py 从 V005_geology_schema.sql 自动生成, 请勿手改。
...
CREATE TABLE coal_seam_def (
    id            INT IDENTITY(1,1) PRIMARY KEY,
    code          VARCHAR(255)    NOT NULL UNIQUE,         -- "4-1"/"9"/"11"/...
    name          VARCHAR(255)    NOT NULL,                -- 显示名 "4-1 号煤层"
    sort_order    INT NOT NULL DEFAULT 0,
    avg_thickness DOUBLE,                            -- 矿区平均厚度 (柱状图比例尺)
    color_hex     VARCHAR(255),                            -- 显示颜色
...
CREATE TABLE coal_classification (
    code        VARCHAR(255)    PRIMARY KEY,               -- WY1/PM/SM/JM/FM/QM/CY/...
    name_cn     VARCHAR(255)    NOT NULL,                  -- 无烟煤一号/贫煤/瘦煤/焦煤
    name_short  VARCHAR(255),                              -- 短名 "无烟/贫/瘦/焦"
    vdaf_min    DOUBLE,
    vdaf_max    DOUBLE,
    g_min       DOUBLE,
    g_max       DOUBLE,
...
CREATE TABLE coal_grade_rule (
    id          INT IDENTITY(1,1) PRIMARY KEY,
    rule_type   VARCHAR(255)    NOT NULL,                  -- ash/sulfur/qnet/volatile
    level_code  VARCHAR(255)    NOT NULL,                  -- extra_low/low/medium/high/extra_high
    level_name  VARCHAR(255)    NOT NULL,                  -- 显示名 "特低灰"/"低灰"/...
    value_min   DOUBLE,
    value_max   DOUBLE,
...
CREATE TABLE borehole (
    id                  INT IDENTITY(1,1) PRIMARY KEY,
    hole_id             VARCHAR(255)    NOT NULL UNIQUE,           -- 孔号 (1610/B2812/J702/ATB15-08)
    x                   DOUBLE    NOT NULL,                  -- 纬距 (CGCS2000, m)
    y                   DOUBLE    NOT NULL,                  -- 经距 (含 37 带号, m)
    z_collar            DOUBLE,                              -- 孔口高程 (m, 黄海)
    depth_total         DOUBLE,                              -- 总孔深 (m)
    terminate_horizon   VARCHAR(255),                              -- 终孔层位 (Ct3/Cb/O)
    drill_date          VARCHAR(255),                              -- 施工时间 (原格式 "1959.5" / "1982.6.27-7.5")
    drill_unit          VARCHAR(255),                              -- 施工单位
    drill_rating        VARCHAR(255),                              -- 钻探评级 甲/乙/丙
    log_rating          VARCHAR(255),                              -- 测井评级
    overall_rating      VARCHAR(255),                              -- 综合评级
    category            VARCHAR(255),                              -- 类别 (2007核实/2014补勘/生产)
    coord_filled        VARCHAR(255)    NOT NULL DEFAULT '原始',   -- 原始/经距补/高程补/+
    coord_fill_basis    VARCHAR(255),                              -- IDW 邻孔依据
    source_page         INT,
    remark              VARCHAR(2000),
...
CREATE TABLE borehole_seam_result (
    id                          INT IDENTITY(1,1) PRIMARY KEY,
    borehole_id                 INT NOT NULL,
    seam_code                   VARCHAR(255)    NOT NULL,
    -- 钻探
    drill_end_depth             DOUBLE,                       -- 钻探-止煤深度 (m)
    drill_seam_thickness        DOUBLE,                       -- 钻探-煤层厚度
    drill_structure             VARCHAR(255),                       -- 钻探-煤层结构
    drill_recovery_rate         DOUBLE,                       -- 钻探-采取率 %
    drill_quality               VARCHAR(255),                       -- 钻探-质量评价 甲/乙/丙
    -- 测井
    log_end_depth               DOUBLE,
    log_seam_thickness          DOUBLE,
    log_structure               VARCHAR(255),
    log_quality                 VARCHAR(255),
    -- 综合采用
    overall_thickness           DOUBLE,                       -- 综合-煤层厚度
    adopted_thickness           DOUBLE,                       -- 综合-采用厚度 (合计/估算厚)
    weathered_coal_thickness    DOUBLE,                       -- 风氧化煤厚
    parting_thickness           DOUBLE,                       -- 夹石厚度
    -- 顶底板 (Type B)
    roof_lithology              VARCHAR(255),
    floor_lithology             VARCHAR(255),
    floor_elevation             DOUBLE,
    overall_rating              VARCHAR(255),                       -- 综合级别 "钻甲"/"测乙"
    -- 状态
    "status"                      VARCHAR(255)    NOT NULL DEFAULT '正常',
        -- 正常/未达/尖灭/全风化/陷落柱/空巷/未测/不取芯/废/合并/参考/风/风化/风氧化
    source_page                 INT,
    source_layout               VARCHAR(255),                       -- A20/A19/B17/D6
    remark                      VARCHAR(2000),
    created_at                  VARCHAR(255)    NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at                  VARCHAR(255)    NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE coal_sample (
    id                  INT IDENTITY(1,1) PRIMARY KEY,
    borehole_id         INT NOT NULL,
    seam_code           VARCHAR(255)    NOT NULL,
    -- 采样位置
    depth_from          DOUBLE,                              -- 采样起深 (m)
    depth_to            DOUBLE,                              -- 采样止深 (m)
    sample_thickness    DOUBLE,                              -- 采样厚度
    z_sample            DOUBLE,                              -- 样品中心高程 (m) — 入库时算好
    -- 密度
    apparent_density    DOUBLE,                              -- 视密度 t/m³
    true_density        DOUBLE,                              -- 真密度 t/m³
    -- 工业分析 原煤
    mad_raw             DOUBLE,                              -- M_ad %
    ad_raw              DOUBLE,                              -- A_d  %
    vdaf_raw            DOUBLE,                              -- V_daf %
    fcd_raw             DOUBLE,                              -- FC_d %
    -- 工业分析 浮煤
    mad_clean           DOUBLE,
    ad_clean            DOUBLE,
    ...
    fcd_clean           DOUBLE,
    -- 全硫
    std_raw             DOUBLE,                              -- 原煤 S_t,d %
    std_clean           DOUBLE,                              -- 浮煤 S_t,d %
    -- 发热量
    qgr_d               DOUBLE,                              -- 干基弹筒 MJ/kg
    qnet_ad             DOUBLE,                              -- 空干基低位 MJ/kg
    -- 胶质层
    plastic_x_mm        DOUBLE,                              -- 胶质层 X mm
    plastic_y_mm        DOUBLE,                              -- 胶质层 Y mm
    plastometric_curve  VARCHAR(255),                              -- 曲线形状/熔合状况
    -- 粘结 + 焦渣
    caking_g            DOUBLE,                              -- 粘结指数 G
    char_residue_raw    INT,                           -- 原煤焦渣 1-8
    char_residue_clean  INT,                           -- 浮煤焦渣 1-8
    -- 洗选 + 煤类
    clean_coal_yield    DOUBLE,                              -- 浮煤回收率 %
    coal_type           VARCHAR(255),                              -- GB/T 5751
    -- 元数据
    source_page         INT,
    remark              VARCHAR(2000),
    created_at          VARCHAR(255)    NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE coal_sample_summary (
    id                  INT IDENTITY(1,1) PRIMARY KEY,
    borehole_id         INT NOT NULL,
    seam_code           VARCHAR(255)    NOT NULL,
    sample_count        INT NOT NULL DEFAULT 0,
    avg_thickness       DOUBLE,
    avg_mad_raw         DOUBLE,
    avg_mad_clean       DOUBLE,
    avg_ad_raw          DOUBLE,
    avg_ad_clean        DOUBLE,
    avg_vdaf_raw        DOUBLE,
    avg_vdaf_clean      DOUBLE,
    avg_fcd_raw         DOUBLE,
    avg_fcd_clean       DOUBLE,
    avg_std_raw         DOUBLE,
    avg_std_clean       DOUBLE,
    avg_qgr_d           DOUBLE,
    avg_qnet_ad         DOUBLE,
    avg_caking_g        DOUBLE,
    avg_plastic_y       DOUBLE,
    avg_clean_yield     DOUBLE,
    dominant_coal_type  VARCHAR(255),
    is_from_source      INT NOT NULL DEFAULT 0,        -- 1=PDF 原"平均"行, 0=程序聚合
    last_built_at       VARCHAR(255)    NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UNIQUE(borehole_id, seam_code),
    FOREIGN KEY (borehole_id) REFERENCES borehole(id) ON DELETE CASCADE ON UPDATE CASCADE,
    FOREIGN KEY (seam_code)   REFERENCES coal_seam_def(code) ON UPDATE CASCADE
);
...
CREATE TABLE coal_observation_point (
    id                  INT IDENTITY(1,1) PRIMARY KEY,
    point_id            VARCHAR(255)    NOT NULL,
    seam_code           VARCHAR(255)    NOT NULL,
    x                   DOUBLE    NOT NULL,
    y                   DOUBLE    NOT NULL,
    original_y_format   INT NOT NULL DEFAULT 8,        -- 8 含带号, 6 无带号(已补 +37000000)
    seam_thickness      DOUBLE,
    coal_structure      VARCHAR(255),
    estimated_thickness DOUBLE,
    floor_elevation     DOUBLE,
    annual_report       VARCHAR(255),                              -- "2014年年报"/"2023年年报"
    source_page         INT,
    remark              VARCHAR(2000),
    created_at          VARCHAR(255)    NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at          VARCHAR(255)    NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE borehole_lithology_segment (
    id              INT IDENTITY(1,1) PRIMARY KEY,
    borehole_id     INT NOT NULL,
    depth_from      DOUBLE    NOT NULL,
    depth_to        DOUBLE    NOT NULL,
    lithology_code  VARCHAR(255)    NOT NULL,                      -- sand/silt/mud/coal/limestone/...
    lithology_name  VARCHAR(255),                                  -- 粗砂岩/细砂岩/泥岩/煤/...
    color_hex       VARCHAR(255),
    pattern         VARCHAR(255),                                  -- SVG pattern key
    description     VARCHAR(2000),
    "source"          VARCHAR(255)    NOT NULL DEFAULT '推断',       -- '附表2推断'/'外部录入'/'人工'
    sort_order      INT NOT NULL DEFAULT 0,
    FOREIGN KEY (borehole_id) REFERENCES borehole(id) ON DELETE CASCADE ON UPDATE CASCADE
);
...
CREATE TABLE coal_audit_finding (
    id              INT IDENTITY(1,1) PRIMARY KEY,
    audit_run_id    VARCHAR(255),                                  -- 一次审核的批次 ID
    sample_id       INT,
    seam_result_id  INT,
    severity        VARCHAR(255)    NOT NULL,                      -- 严重/警告/信息
    category        VARCHAR(255)    NOT NULL,                      -- 工分自洽/煤类反推/同层离群/...
    message         VARCHAR(255)    NOT NULL,
    suggestion      VARCHAR(255),
    "status"          VARCHAR(255)    NOT NULL DEFAULT 'open',       -- open/processed/ignored
    processed_at    VARCHAR(255),
    processed_by    VARCHAR(255),
    process_note    VARCHAR(2000),
    created_at      VARCHAR(255)    NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
-- 煤层定义 (按沉积深度排序，浅 → 深)
INSERT IGNORE INTO coal_seam_def(code, name, sort_order, color_hex, description) VALUES
...
INSERT IGNORE INTO coal_classification(code, name_cn, name_short, vdaf_min, vdaf_max, g_min, g_max, y_min, y_max, sort_order, description) VALUES
...
INSERT IGNORE INTO coal_grade_rule(rule_type, level_code, level_name, value_min, value_max, color_hex, sort_order) VALUES
    -- 灰分 (GB/T 15224.1)
