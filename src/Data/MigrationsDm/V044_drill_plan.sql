-- 由 build/sqlite2dm.py 从 V044_drill_plan.sql 自动生成, 请勿手改。
...
CREATE TABLE drill_plan (
    equipment_id      VARCHAR(255) NOT NULL,                   -- 钻机编号(= equipment.equipment_id)
    plan_date         VARCHAR(255) NOT NULL,                   -- 作业日期 yyyy-MM-dd
    start_time        VARCHAR(255) NOT NULL,                   -- 起 HH:mm
    end_time          VARCHAR(255) NOT NULL,                   -- 止 HH:mm(同日内;跨零点拆两条)
    "zone"              VARCHAR(255) NOT NULL DEFAULT '',        -- 待爆区/平盘(对 blast_event.location_code 或作业面名)
    bench_elevation_m DOUBLE,                            -- 台阶标高 m(NULL=未录,0 是合法值)
    hole_count        INT,                         -- 计划孔数(NULL=未录)
    hole_length_m     DOUBLE,                            -- 计划延米 m(NULL=未录)
    "status"            VARCHAR(255) NOT NULL DEFAULT '计划',    -- 计划 / 进行中 / 完成 / 取消
    note              VARCHAR(2000),
...
CREATE INDEX idx_drill_plan_date ON drill_plan(plan_date);

-- 按待爆区回查（钻爆衔接：这个区的孔谁在打、打完没有）
CREATE INDEX idx_drill_plan_zone ON drill_plan("zone");
