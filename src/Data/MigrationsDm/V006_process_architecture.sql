-- 由 build/sqlite2dm.py 从 V006_process_architecture.sql 自动生成, 请勿手改。
...
CREATE TABLE process_system (
    system_id        INT IDENTITY(1,1) PRIMARY KEY,
    code             VARCHAR(255) NOT NULL UNIQUE,        -- 系统编码 'blasting' / 'mining' / ...
    name             VARCHAR(255) NOT NULL,                -- 显示名 穿爆系统 / 采装系统 / ...
    category         VARCHAR(255),                          -- 主类:drilling/mining/transport/dumping/slope/safety/aux/economic
    description      VARCHAR(2000),
    display_order    INT NOT NULL DEFAULT 0,    -- UI 排序
...
CREATE TABLE process_phase (
    ...
    code                           VARCHAR(255) NOT NULL,    -- 'drilling' / 'charging' / 'blasting_exec' / ...
    name                           VARCHAR(255) NOT NULL,    -- 显示名 钻孔 / 装药 / 起爆 / ...
    sequence_order                 INT NOT NULL DEFAULT 0,   -- 系统内环节先后顺序
    typical_equipment_category     VARCHAR(255),             -- 该环节典型适用设备类别 Drill/Shovel/Truck/Dozer/...
    description                    VARCHAR(2000),
    is_active                      INT NOT NULL DEFAULT 1,
    created_at                     VARCHAR(255) NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at                     VARCHAR(255) NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE parameter_definition (
    ...
    code                 VARCHAR(255) NOT NULL UNIQUE,     -- 'hole_diameter' / 'hole_depth' / ...(全表唯一,供公式引用)
    name                 VARCHAR(255) NOT NULL,             -- 显示名 孔径 / 孔深 / ...
    "unit"                 VARCHAR(255),                       -- 单位 mm / m / kg / kg/m³ / °  / ...
    value_type           VARCHAR(255) NOT NULL DEFAULT 'numeric' CHECK (value_type IN ('numeric','text','boolean','enum')),
    standard_min         DOUBLE,                       -- 标准范围下限
    standard_max         DOUBLE,                       -- 标准范围上限
    standard_default     DOUBLE,                       -- 默认推荐值
    alarm_low            DOUBLE,                       -- 报警下限(超低则红)
    alarm_high           DOUBLE,                       -- 报警上限
    is_required          INT NOT NULL DEFAULT 0, -- 是否必填
    calc_formula         VARCHAR(2000),                       -- 派生公式(可空,如 "explosive_kg / blast_volume_m3")
    source_table         VARCHAR(255),                       -- 可派生自哪张事实表(可空)
    source_column        VARCHAR(255),                       -- 派生字段
    description          VARCHAR(2000),
...
CREATE TABLE equipment_constraint (
    ...
    equipment_model   VARCHAR(255) NOT NULL,                 -- equipment_model.model FK
    constraint_type   VARCHAR(255) NOT NULL CHECK (constraint_type IN ('min','max','range','equals','contains')),
    limit_value       DOUBLE,                          -- 单值约束
    limit_min         DOUBLE,                          -- 范围约束下限
    limit_max         DOUBLE,                          -- 范围约束上限
    text_value        VARCHAR(255),                          -- 文本/枚举型约束
    consequence       VARCHAR(255) NOT NULL DEFAULT 'hard' CHECK (consequence IN ('hard','soft','informational')),
                                                    -- hard: 违反→不可用
                                                    -- soft: 违反→降效
                                                    -- informational: 仅提示
    priority          INT NOT NULL DEFAULT 3,    -- 1-5,5 最严
    description       VARCHAR(2000),
    is_active         INT NOT NULL DEFAULT 1,
    created_at        VARCHAR(255) NOT NULL DEFAULT CURRENT_TIMESTAMP,
