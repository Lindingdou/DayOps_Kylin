-- 由 build/sqlite2dm.py 从 V042_maintenance_window.sql 自动生成, 请勿手改。
...
CREATE TABLE maintenance_window (
    equipment_id  VARCHAR(255) NOT NULL,                       -- 设备编号(= equipment.equipment_id)
    plan_date     VARCHAR(255) NOT NULL,                       -- 检修日期 yyyy-MM-dd
    start_time    VARCHAR(255) NOT NULL,                       -- 起 HH:mm
    end_time      VARCHAR(255) NOT NULL,                       -- 止 HH:mm(同日内;跨零点拆两条)
    kind          VARCHAR(255) NOT NULL DEFAULT '定修',        -- 定修 / 保养 / 临修 / 年检
    note          VARCHAR(2000),                                -- 备注(检修内容、承修班组)
