-- 由 build/sqlite2dm.py 从 V040_dump_strip_sub_index.sql 自动生成, 请勿手改。
...
ALTER TABLE dump_strip ADD COLUMN sub_index INT NOT NULL DEFAULT 0;   -- 带内子号(1 起;0 = 该带没切)
ALTER TABLE dump_strip ADD COLUMN sub_count INT NOT NULL DEFAULT 1;   -- 本带共切成几个位置

DROP INDEX IF EXISTS idx_dump_strip_key;

-- 一个排土场在一个方案下,同一(级,幅,带,子号)只能有一行(方案为空视作默认方案)
CREATE UNIQUE INDEX idx_dump_strip_key
    ON dump_strip(region_id, NVL(design_version, ''), level_index, panel_index, step_index, sub_index);
