-- 由 build/sqlite2dm.py 从 V030_virtual_drill.sql 自动生成, 请勿手改。
...
CREATE TABLE virtual_drill_surface (
    id             INT IDENTITY(1,1) PRIMARY KEY,
    "role"           VARCHAR(255)    NOT NULL DEFAULT 'roof',      -- 'surface' | 'roof' | 'floor'
    seam_name      VARCHAR(255)    NOT NULL DEFAULT '',          -- 地表为空;顶/底板为煤层编号(如 4 / 7-1 / 9 / 11)
    seam_order     INT NOT NULL DEFAULT 0,           -- 地质排序(自上而下递增,0 在最上)
    color_hex      VARCHAR(255)    NOT NULL DEFAULT '#3C3C3C',   -- 层位显示色(可配置;#RRGGBB)
    source_layer   VARCHAR(255)    NOT NULL DEFAULT '',          -- 来源:原场景三角网图层名(溯源)
    vertex_count   INT NOT NULL DEFAULT 0,
    triangle_count INT NOT NULL DEFAULT 0,
    min_x DOUBLE NOT NULL DEFAULT 0,
    min_y DOUBLE NOT NULL DEFAULT 0,
    min_z DOUBLE NOT NULL DEFAULT 0,
    max_x DOUBLE NOT NULL DEFAULT 0,
    max_y DOUBLE NOT NULL DEFAULT 0,
    max_z DOUBLE NOT NULL DEFAULT 0,
    geometry_b64   VARCHAR(2000)    NOT NULL DEFAULT '',          -- 打包的顶点 + 三角索引(base64,opaque)
