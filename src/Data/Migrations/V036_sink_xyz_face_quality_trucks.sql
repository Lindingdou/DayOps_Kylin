...
ALTER TABLE sink_profile ADD COLUMN x REAL NOT NULL DEFAULT 0;   -- 去向代表点 X(m);x、y 均为 0 = 未录坐标
ALTER TABLE sink_profile ADD COLUMN y REAL NOT NULL DEFAULT 0;   -- 去向代表点 Y(m)
ALTER TABLE sink_profile ADD COLUMN z REAL NOT NULL DEFAULT 0;   -- 去向代表点 Z 标高(m);Z=0 是合法标高,不参与"有没有坐标"的判定

-- ── ② 作业面煤质目标(配煤约束输入;NULL = 该面无煤质目标,即纯量矿)──────────────
ALTER TABLE working_face_routing ADD COLUMN quality_ash_pct      REAL;   -- 灰分目标 %(NULL=无目标)
ALTER TABLE working_face_routing ADD COLUMN quality_cv_mjkg      REAL;   -- 热值目标 MJ/kg(NULL=无目标)
ALTER TABLE working_face_routing ADD COLUMN quality_sulfur_pct   REAL;   -- 硫分目标 %(NULL=无目标)
ALTER TABLE working_face_routing ADD COLUMN quality_moisture_pct REAL;   -- 水分目标 %(NULL=无目标)
