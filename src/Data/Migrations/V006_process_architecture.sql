...
AFTER UPDATE ON process_system FOR EACH ROW BEGIN
    UPDATE process_system SET updated_at = CURRENT_TIMESTAMP WHERE system_id = NEW.system_id;
END;
...
CREATE INDEX IF NOT EXISTS idx_phase_system ON process_phase(system_id);

CREATE TRIGGER IF NOT EXISTS trg_process_phase_updated_at
AFTER UPDATE ON process_phase FOR EACH ROW BEGIN
    UPDATE process_phase SET updated_at = CURRENT_TIMESTAMP WHERE phase_id = NEW.phase_id;
END;
...
CREATE TABLE IF NOT EXISTS parameter_definition (
    ...
    standard_min         REAL,                       -- 标准范围下限
    standard_max         REAL,                       -- 标准范围上限
    standard_default     REAL,                       -- 默认推荐值
    alarm_low            REAL,                       -- 报警下限(超低则红)
...
CREATE INDEX IF NOT EXISTS idx_param_phase ON parameter_definition(phase_id);
CREATE INDEX IF NOT EXISTS idx_param_code  ON parameter_definition(code);

CREATE TRIGGER IF NOT EXISTS trg_parameter_definition_updated_at
AFTER UPDATE ON parameter_definition FOR EACH ROW BEGIN
    UPDATE parameter_definition SET updated_at = CURRENT_TIMESTAMP WHERE param_id = NEW.param_id;
END;
...
CREATE TABLE IF NOT EXISTS equipment_constraint (
    ...
    limit_value       REAL,                          -- 单值约束
    limit_min         REAL,                          -- 范围约束下限
    limit_max         REAL,                          -- 范围约束上限
...
CREATE INDEX IF NOT EXISTS idx_constraint_param ON equipment_constraint(param_id);
CREATE INDEX IF NOT EXISTS idx_constraint_model ON equipment_constraint(equipment_model);
