...
CREATE INDEX IF NOT EXISTS idx_maintenance_window_date
    ON maintenance_window(plan_date);

CREATE TRIGGER IF NOT EXISTS trg_maintenance_window_updated_at
AFTER UPDATE ON maintenance_window FOR EACH ROW BEGIN
    UPDATE maintenance_window SET updated_at = CURRENT_TIMESTAMP
