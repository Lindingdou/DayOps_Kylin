...
ALTER TABLE working_face_routing ADD COLUMN available_reserve_m3 REAL NOT NULL DEFAULT 0;   -- 备采储量(m³ 原位实方);0=未录,非"采空"
ALTER TABLE working_face_routing ADD COLUMN advance_azimuth_deg  REAL;                      -- 推进方位(°,正北起顺时针);NULL=未录
ALTER TABLE working_face_routing ADD COLUMN mining_width_m       REAL;                      -- 本面当前推进宽(m);NULL=未录

-- 按单元号回查作业面（点图上的体 → 找到今天是哪个面在采它）
CREATE INDEX IF NOT EXISTS idx_working_face_routing_unit
