...
CREATE TABLE IF NOT EXISTS dump_strip (
    ...
    crest_z             REAL    NOT NULL,           -- 坡顶标高(m)
    toe_z               REAL    NOT NULL,           -- 坡底标高(m)
    bench_height_m      REAL    NOT NULL,           -- 台阶高 = crest_z − toe_z

    strike_len_m        REAL    NOT NULL,           -- 走向长(m,水平)
    strip_width_m       REAL    NOT NULL,           -- 排土条带宽度 W(m,推进方向)
    capacity_m3         REAL    NOT NULL,           -- 库容(m³,占容方)= 走向长 × W × 台阶高

    centroid_x          REAL    NOT NULL,           -- 质心(世界坐标)—— 寻径算运距吃它
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_dump_strip_key
    ON dump_strip(region_id, IFNULL(design_version, ''), level_index, panel_index, step_index);

-- 按排土场取清单 / 按台阶级取清单,是最常走的两条读法
CREATE INDEX IF NOT EXISTS idx_dump_strip_region ON dump_strip(region_id, level_index, step_index);

CREATE TRIGGER IF NOT EXISTS trg_dump_strip_updated_at
AFTER UPDATE ON dump_strip FOR EACH ROW BEGIN
    UPDATE dump_strip SET updated_at = CURRENT_TIMESTAMP WHERE id = NEW.id;
END;
