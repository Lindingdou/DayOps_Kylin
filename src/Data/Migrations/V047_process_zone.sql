...
CREATE TABLE IF NOT EXISTS process_zone (
    ...
    cell_m             REAL NOT NULL,                  -- 栅格格距 m = 轮廓精度
    area_m2            REAL NOT NULL DEFAULT 0,        -- 栅格面积(不是描边环面积)
    ring_area_m2       REAL NOT NULL DEFAULT 0,        -- 描边环面积(与上面差得多 = 描边不可信)
    clipped_at_border  INTEGER NOT NULL DEFAULT 0,     -- 1 = 膨胀顶到格网边界，本区被截断

    unit_ids           TEXT NOT NULL DEFAULT '',       -- 覆盖的采掘单元号，顿号分隔
    unit_count         INTEGER NOT NULL DEFAULT 0,
    volume_m3          REAL,                           -- 本区的量(NULL=不适用,如警戒区)
    volume_basis       TEXT NOT NULL DEFAULT '',       -- 量口径:原位实方/控制方量/排弃占容(三者绝不许并成一列)
    equip_role         TEXT NOT NULL DEFAULT '',       -- 该区的设备角色:钻机/电铲/卡车/推土机

    lead_days          REAL,                           -- 相对采装的超前(+)/滞后(-)工日;NULL=不适用
    guard_radius_m     REAL,                           -- 警戒半径 m(仅 blast_guard)
    band_width_m       REAL,                           -- 带宽 m(仅 dump_tip/dump_doze)
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_process_zone_key
    ON process_zone(period, process, name);

-- 按期取一整期(界面每次开窗问一次)
CREATE INDEX IF NOT EXISTS idx_process_zone_period ON process_zone(period);
-- 按工序回查(派工:今天钻机该去哪几块地)
CREATE INDEX IF NOT EXISTS idx_process_zone_process ON process_zone(process, period);
-- 按作业面回查(一个面的五道工序摆在一起看)
CREATE INDEX IF NOT EXISTS idx_process_zone_group ON process_zone(group_key);

CREATE TRIGGER IF NOT EXISTS trg_process_zone_updated_at
AFTER UPDATE ON process_zone FOR EACH ROW BEGIN
    UPDATE process_zone SET updated_at = CURRENT_TIMESTAMP WHERE id = NEW.id;
END;
