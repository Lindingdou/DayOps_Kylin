...
CREATE TABLE IF NOT EXISTS seam_bench_param (
    ...
    bench_height_m          REAL,                           -- 覆盖 coal_bench_height (2007)
    bench_slope_angle_deg   REAL,                           -- 覆盖 coal_bench_slope_angle (2008)
    berm_width_m            REAL,                           -- 覆盖 coal_platform_width (2009)

    -- 煤的采矿模型专有(parameter_definition 里没有对应项)
    strip_width_m           REAL,                           -- 采掘带宽度 W(往高墙里推进的水平深度)
    layering_mode           TEXT    NOT NULL DEFAULT 'inclined',  -- 'inclined' 倾斜分层 / 'horizontal' 顶底水平
    min_mineable_thick_m    REAL,                           -- 最小可采厚:薄于此不出煤体
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_seam_bench_param_key
    ON seam_bench_param(seam_code, IFNULL(design_version, ''));

CREATE INDEX IF NOT EXISTS idx_seam_bench_param_active
    ON seam_bench_param(is_active);

CREATE TRIGGER IF NOT EXISTS trg_seam_bench_param_updated_at
AFTER UPDATE ON seam_bench_param FOR EACH ROW BEGIN
    UPDATE seam_bench_param SET updated_at = CURRENT_TIMESTAMP WHERE id = NEW.id;
END;
...
INSERT OR IGNORE INTO seam_bench_param
