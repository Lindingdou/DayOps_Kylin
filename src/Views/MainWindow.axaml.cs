...
public partial class MainWindow : Window
{
    ...
    private async void OnRibbonCommand(object? sender, RoutedEventArgs e)
    {
        if (sender is Control c && c.Tag is string cmd)
        {
            ...
            if (cmd == "坡度着色" || cmd == "三角网着色") { await ShadeTinAsync("坡度着色", "绿=平 → 红=陡", TerrainAnalysis.BuildSlopeMap); return; }
    ...
    private static readonly string[] CommandCatalog =
    {
        ...
        // §四/§八 数据分析(SQLite 种子库)
        "设备台账","生产数据","产能分析","故障分析","KPI分析","设备智能编组","钻孔管理","煤质统计","煤层管理","工艺架构","展绘层位数据","导入生产记录","导入月度产能","导入故障记录","导入月度KPI","导入设备台账","导入煤质","导入观测点",
